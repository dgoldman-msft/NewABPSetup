...
$parentModule = 'NewABPSetup'
if (-not $ModuleName) { $ModuleName = 'NewABPSetup.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\NewABPSetup'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\NewABPSetup.psd1"
$prereqHash = @{
