...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'NewABPSetup.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'NewABPSetup.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'NewABPSetup.Client.ProtectedToken'
